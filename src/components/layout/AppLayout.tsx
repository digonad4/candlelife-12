...
const AppLayout = () => {
  ...
  return <div className="min-h-screen flex flex-col bg-background text-foreground overflow-hidden w-full">
      <div className="flex flex-1 overflow-hidden relative h-full w-full">
        ...
        <main className="flex-1 flex flex-col overflow-auto transition-all duration-300 w-full h-full">
          ...
          <Button variant="ghost" size="icon" onClick={toggleSidebar} aria-label="Toggle Sidebar" className="fixed top-5 left-8 z-40 my-[49px]">
      ...
      <ChatModal isOpen={isChatOpen} onOpenChange={setIsChatOpen} recipientId={chatRecipient.id} recipientName={chatRecipient.name} recipientAvatar={chatRecipient.avatar} />
    </div>;
};
